...
| `Combobox` \* | `bar`, `column`, `data`, `filter_bar`, `label`, `limit`, `min_chars`, `multi`, `name`, `placeholder`, `url_sync` | Searchable single-select filter for a **high-cardinality** column — type-to-filter over thousands of customers / SKUs / users where a plain `<Dropdown>` (which loads *every* distinct value) would cho… |
...
| `RangeSlider` \* | `bar`, `currency`, `date_format`, `decimals`, `default`, `filter_bar`, `format`, `label`, `locale`, `max`, `max_param`, `min`, `min_param`, `name`, `step`, `url_sync` | Numeric range filter — a dual-handle slider for a *between* bound on a numeric column (price, age, score, …). |
...
| `Slider` \* | `bar`, `currency`, `date_format`, `decimals`, `default`, `filter_bar`, `format`, `label`, `locale`, `max`, `min`, `name`, `step`, `url_sync` | Single-value numeric **threshold** filter — one handle on a track for a ``min rating ≥``, ``price ≤``, ``top N`` style bound. The one-handled sibling of ``<RangeSlider>`` (which carries a low/high pa… |
