...
|---|---|---|
| `Ask` | `ask`, `by`, `cache_ttl`, `col-span`, `data`, `grain`, `highlight`, `inline`, `label`, `lazy`, `max_rows`, `metric`, `refresh`, `replay`, `series`, `span` | Pin a question to a query's result and render the LLM's answer as a prose block alongside the charts. |
